...
Hook 分两种类型，详见 [prompt-hook.md](prompt-hook.md)：

- `"type": "command"` — 执行 Shell 脚本，exit code 控制放行/拦截
- `"type": "prompt"` — 用自然语言描述规则，由 Claude 判断是否放行

## 本目录示例

| 文件 | 事件 | 功能 |
|------|------|------|
| `block-rm.sh` | PreToolUse | 拦截 `rm -rf` 命令（Shell Hook） |
| `protect-files.sh` | PreToolUse | 保护 `.env`、`package-lock.json`、`.git/` 不被修改 |
| `command-log.sh` | PreToolUse/Bash | 记录每次 Bash 命令到日志 |
| `echo.sh` | SessionStart | 会话启动时追加日志 |
| `hook-input-dump.json` | — | Hook stdin JSON 结构示例 |
| `prompt-hook.md` | — | Prompt Hook 学习笔记（含演示） |
